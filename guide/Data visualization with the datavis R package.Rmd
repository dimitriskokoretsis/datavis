...
The full documentation of `datavis` functions is available in PDF format here: [datavis_0.0.1.pdf](https://github.com/dimitriskokoretsis/datavis/raw/master/guide/datavis_0.0.1.pdf)
...
A good elaboration of these arguments, as well as a basic introduction to R can be found in this blog post: [Excel vs R: A Brief Introduction to R](https://www.jessesadler.com/post/excel-vs-r/).
...
R comes with its own integrated development environment (IDE), but it is quite limiting. **RStudio** is the most popular IDE for R. It is free and offers many advantages over the default R IDE. Download and install the desktop version it through this website: <https://www.rstudio.com/>.
...
```{r,eval=FALSE}
# Do not copy and paste this command. Substitute quoted part with the path to your project folder.
...
Download the following zip file with mock data and extract its contents in the project's working directory: [demo_data.zip](<https://github.com/dimitriskokoretsis/datavis/raw/master/demo_data.zip>). This should create a subfolder named "demo_data" with 2 CSV files.
...
In R, a vector is a group of elements of the same type, arranged in a specific order. We can create a vector in R as follows:

```{r}
d <- c(2,7,3,6,4)
```

`c()` is a special function that creates a vector with the elements inside the brackets. So this piece of code creates a variable named `d` and assigns to it the values (2,7,3,6,4) in this specific order.
...
A data frame is a table structure with values organized in rows (*records*) and columns (*fields*). It is the type of data that most resembles a spreadsheet. Each column has values of the same type (for example, `numeric`). Because of this, each column can be conveniently used in calculations as a vector.

R has a built-in type of data structure called `data.frame`. The functions described in this guide are based on an R package called [`data.table`](https://rdatatable.gitlab.io/data.table/), which offers a more convenient syntax than base R. This means that the `data.table` package needs to be installed in your system, but the functions can also accept standard R `data.frame`s as input.

The `fread` function of the `data.table` package can read any text file of table form into a data frame.
...
```{r,eval=FALSE}
h |>
  function.1(input.1 = g, _) |>
  function.2(input.1 = k, _)
```
